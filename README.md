...
- **Coverage is deliberately partial.** 255 specs cover tenant isolation, the
...
- **The browser is covered at six pages, not across the app.** Fourteen system
  specs drive headless Chromium through Capybara and Cuprite: the landing page,
  sign-in through a tenant subdomain to the dashboard, the HR leave queue, the
  HR leave form and its select2 control, the analytics charts, and a generated
  payslip. An uncaught JavaScript exception on any page they load fails the
  build, and they assert the globals — jQuery, select2, Bootstrap, Chartkick —
  that the rest of the front end reads off `window`. Every page they do not
  load is still covered by nothing that runs the JavaScript, which is why the
  remaining front-end updates are taken one at a time, each behind specs
  reaching what it touches.
